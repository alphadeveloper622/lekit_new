\@extends('admin.partials.master')
    @php
        $title = isset($edit) ? trans('Banner Edit') : __('Banner Add')
    @endphp
@section('title')
    {{ $title }}
@endsection
                ...
                <div class="col-sm-xs-12 col-md-10 middle">
                    <div class="card">
                        <div class="card-header input-title">
                            <h4>{{ isset($edit) ? trans('Edit Banner') : __('Add New Banner')}}</h4>
                        </div>
                        <div class="card-body card-body-paddding">
                            @php
                                $route = isset($edit) ? route('banners.update',$edit->id) : route('banners.store')
                            @endphp
                            ...
                            <form method="POST" action="{{ $route }}">
                                ...
                                @php
                                    $size = addon_is_activated('ishopet') ? __('192X170') : __('220x125');
                                @endphp
                                <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group gallery-modal" id="btnSubmit" data-for="image"
                                            ...
                                            <input type="hidden" name="image" value="{{ isset($edit) ? $edit->image_id : '' }}"
                                                   class="image-selected">
                                            <span class="form-control"><span
                                                    class="counter">{{ isset($edit) && $edit->image_id ? 1 : 0 }}</span> {{ __('file chosen') }}</span>
                                        ...
                                        <div class="selected-media-box">
                                            <div class="mt-2 gallery gallery-md d-flex">
                                                <div class="selected-media mr-2 mb-2 mt-3 ml-0"
                                                    ...
                                                    @if (isset($edit) && $edit->image_id && array_key_exists('image_72x72',$edit->image) && is_file_exists($edit->image['image_72x72'], $edit->image['storage']))
                                                        <img
