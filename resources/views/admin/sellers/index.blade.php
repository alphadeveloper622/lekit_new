                                ...
                                <form class="form-inline" id="sorting">
                                    <div class="form-group">
                                        <select class="form-control selectric sorting" name="a">
                                            ...
                                            <option
                                                    {{ @$a == "banned" ? "selected" : "" }} value="banned">{{ __('Banned Seller') }}</option>
                                        </select>
                                    </div>
                                        ...
                                        <tr id="row_{{$user->id}}">
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <figure class="avatar-company mr-2">
                                                        @if (@$user->sellerProfile->logo != [] && @is_file_exists($user->sellerProfile->logo['image_72x72'],$user->sellerProfile->logo['storage']))
                                                            <a target="{{ isAppMode() ? '_parent' : '_blank'}}"
                                                               href="{{ isAppMode() ? '#' : (isset($user->sellerProfile->company_website) ? $user->sellerProfile->company_website : '') }}">
                                                                <img src="{{ get_media($user->sellerProfile->logo['image_72x72'],$user->sellerProfile->logo['storage']) }}"
                                                                     alt="{{ $user->sellerProfile->company_name }}">
                                                            </a>
                                                        @else
                                                            <a target="{{ isAppMode() ? '_parent' : '_blank'}}"
                                                               href="{{ isAppMode() ? '#' : (isset($user->sellerProfile->company_website) ? $user->sellerProfile->company_website : '') }}">
                                                    ...
                                                    <div class="ml-1">
                                                        <a target="{{ isAppMode() ? '_parent' : '_blank'}}"
                                                           href="{{ isAppMode() ? '#' : (isset($user->sellerProfile->company_website) ? $user->sellerProfile->company_website : '') }}"> {{ !blank(@$user->sellerProfile->company_name) ? $user->sellerProfile->company_name : '' }}</a><br/>                                                        
