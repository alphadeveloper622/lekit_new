    ...
    <section class="section">
        <div class="section-body ">
            <div class="d-flex justify-content-between">
                <div class="d-block">
                    <h2 class="section-title">{{__('offline_purchase_histories')}}</h2>
                                ...
                                <table class="table table-striped table-md">
                                    ...
                                    @foreach($purchases as $key => $purchase)
                                        <tr id="row_{{ $key+1 }}" class="table-data-row">
                                            ...
                                            <td class="text-capitalize">{{ $purchase->offline->getTranslation('name') }}</td>
                                            <td>{{ get_price($purchase->amount) }}</td>
                                            <td>{{ $purchase->trx_id }}</td>
                                            <td>{{ Carbon\Carbon::parse($purchase->purchase_at)->format('d M Y h:i A') }}</td>
                                            <td>
                                                @if($purchase->offline_method_file)
                                                    <a target="_blank" href="{{ get_media($purchase->offline_method_file['image'],$purchase->offline_method_file['storage']) }}">{{ explode('/',$purchase->offline_method_file['image'])[1] }}</a>
                                                @endif
                                            </td>
                                                    ...
                                                    @if(authUser()->user_type == 'admin')
                                                        <label class="custom-switch mt-2 {{ hasPermission('package_status_change') ? '' : 'cursor-not-allowed' }}">
                                                            <input type="checkbox" name="custom-switch-checkbox"
                                                                   ...
                                                                   {{ $purchase->status == 1 ? 'checked' : '' }} {{ hasPermission('package_status_change') ? '' : 'disabled' }} class="{{ hasPermission('package_status_change') ? 'subscription-status-change' : '' }} custom-switch-input">
